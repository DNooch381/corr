...
void plotfit(TProfile* histogram, TF1* fun, const char* handle)
{
  ...
  histogram->GetYaxis()->SetTitle(Form("#LT%s#GT",handle));
  ...
  //c1->Print(Form("Figures/histogramR_%s_100.png",handle));

  c1->SetLogy(1);
  histogram->Draw();
  //tex->DrawLatex(xtex,ytex,"100logy");
  //c1->Print(Form("Figures/histogramR_%s_100logy.png",handle));

  c1->SetLogx(1);
  histogram->Draw();
  //tex->DrawLatex(xtex,ytex,"100logx/y");
  //c1->Print(Form("Figures/histogramR_%s_100logylogx.png",handle));
  ...
  c1->Print(Form("Figures/histogramR_%s_fit_100logylogx.png",handle));
  ...
  ytex = 0.73;
  //tex->DrawLatex(xtex,ytex,Form("#chi^{2}/NDF = %.2f/%d",chi2,ndf));
  c1->Print(Form("Figures/histogramR_%s_fit_100logy.png",handle));
  ...
  ytex = 0.73;
  //tex->DrawLatex(xtex,ytex,Form("#chi^{2}/NDF = %.2f/%d",chi2,ndf));
  //c1->Print(Form("Figures/histogramR_%s_fit_100.png",handle));
